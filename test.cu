...
void STLU_GPU(int dlen, int threadnum, uint32_t *d_database, uint32_t *d_params, uint32_t *d_decbit, cuDoubleComplex *d_twist, cuDoubleComplex *d_id, Params128 &p128)
{
    int M = p128.N / 2;
    int lines = 2 * p128.bk_l;
    int idsize = lines * p128.N;
...
float Test_QUERY_GPU()
{
    ...
    // 查表一共需要 dlen 次，所以循环 dlen 次
    // int threadnum = flen * p128.N;
    // STLU_GPU(dlen, threadnum, d_database, d_params, d_decbit, d_twist, d_id, p128);
    // begin to count time
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    ...
    for (int num = 0; num < N_GPU; num++)
    {
        ...
        CHECK(cudaMemcpyAsync(d_database, h_database, flen * 2 * p128.N * sizeof(uint32_t), cudaMemcpyHostToDevice, stream[num]));
...
void Test_Solution()
{
    Params128 p128 = Params128(16, 8, 16, 2, 10, 4, 9, 2, 8, 10, 3, pow(2.0, -15.3), pow(2.0, -28), pow(2.0, -31), pow(2.0, -44), 2);
    ...
    // 要查找第 idnum = 35 条数据， GetBits 把 idnum 转成二进制并逆置，得到 idbits =[1, 1, 0, 0, 0, 1]
    int idnum = 111;
    uint32_t *idbits = (uint32_t *)malloc((dlen + 1) * sizeof(uint32_t));
    GetBits(idnum, idbits, dlen + 1);
    ...
    for (int num = 0; num < N_GPU; num++)
    {
        ...
        cuDoubleComplex *d_twist;
        CHECK(cudaMalloc((void **)&d_twist, M * sizeof(cuDoubleComplex)));
        CHECK(cudaMemcpyAsync(d_id, h_id, dlen * lines * p128.N * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice, stream[num]));
        CHECK(cudaMemcpyAsync(d_params, h_params, 4 * sizeof(uint32_t), cudaMemcpyHostToDevice, stream[num]));
        CHECK(cudaMemcpyAsync(d_decbit, p128.decbit, p128.bk_l * sizeof(uint32_t), cudaMemcpyHostToDevice, stream[num]));
        CHECK(cudaMemcpyAsync(d_twist, p128.twist, M * sizeof(cuDoubleComplex), cudaMemcpyHostToDevice, stream[num]));
        ...
        {
            uint32_t *h_t1, *d_t1;
            CHECK(cudaMallocHost((void **)&h_t1, threadnum * 2 * sizeof(uint32_t)));
            CHECK(cudaMalloc((void **)&d_t1, threadnum * 2 * sizeof(uint32_t)));
            cuDoubleComplex *h_decvecfft, *d_decvecfft;
            CHECK(cudaMallocHost((void **)&h_decvecfft, threadnum * sizeof(cuDoubleComplex)));
            CHECK(cudaMalloc((void **)&d_decvecfft, threadnum * sizeof(cuDoubleComplex)));
            cuDoubleComplex *h_t4, *d_t4;
            CHECK(cudaMallocHost((void **)&h_t4, 2 * threadnum * sizeof(cuDoubleComplex)));
            CHECK(cudaMalloc((void **)&d_t4, 2 * threadnum * sizeof(cuDoubleComplex)));
            cuDoubleComplex *h_t5, *d_t5;
            CHECK(cudaMallocHost((void **)&h_t5, threadnum / 2 * sizeof(cuDoubleComplex)));
            CHECK(cudaMalloc((void **)&d_t5, threadnum / 2 * sizeof(cuDoubleComplex)));
            uint32_t *h_t6, *d_t6;
            CHECK(cudaMallocHost((void **)&h_t6, threadnum * sizeof(uint32_t)));
            CHECK(cudaMalloc((void **)&d_t6, threadnum * sizeof(uint32_t)));
            ...
            CHECK(cudaFree(d_t1));
            CHECK(cudaFreeHost(h_t1));
            CHECK(cudaFree(d_decvecfft));
            CHECK(cudaFreeHost(h_decvecfft));
            CHECK(cudaFree(d_t4));
            CHECK(cudaFreeHost(h_t4));
            CHECK(cudaFree(d_t5));
            CHECK(cudaFreeHost(h_t5));
            CHECK(cudaFree(d_t6));
            CHECK(cudaFreeHost(h_t6));
        }
        
        // 这里其实只需要把最后的 2 * N 个数据传过来即可，然后放到二维数组 c 中，调用trlweSymDec进行解密
        CHECK(cudaMemcpyAsync(h_database, d_database, flen * 2 * p128.N * sizeof(uint32_t), cudaMemcpyDeviceToHost));
        uint32_t **c = (uint32_t **)malloc(sizeof(uint32_t *) * 2);
        for (int i = 0; i < 2; i++)
        {
            c[i] = (uint32_t *)malloc(sizeof(uint32_t) * p128.N);
...
int main()
{
    ...
    //     cout << "\n" << i << ": " << double(end - start) / CLOCKS_PER_SEC * 1000 << "ms" << endl;
    // }
    // Test_MHomOR();
    // Test_QUERY_GPU();
    Test_Solution();
